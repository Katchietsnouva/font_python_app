...
def create_login_window():
    ...
    def check_fields(*args):
        ...
        register_button.configure(state="normal" if all_filled else "disabled")
    ...
    def getvals():
        ...
        with open(file_path, "r+") as json_file:
            ...
            if customer_data["Name"] in existing_names:
                messagebox.showerror("Data Present  Error", f"Customer with name '{customer_data['Name']}' already exists.")
                load_user_data( ) # to repopuate the entry fields as return clears them
                return  # Don't proceed if the name already exists
    ...
    def getvals_login():
        ...
        with open(file_path, "r") as json_file:
            ...
            if customer_data["Name"] in existing_names:
                ...
                messagebox.showerror("Data not Present Error", f"Register first! Customer with name '{customer_data['Name']}' doesnt exists.")
                return
            # check_fields()
    ...
    # Register button
    register_button = CTkButton(group3, text="Register", command=getvals, state="disabled")
    register_button.pack(side=tk.LEFT, padx=5, pady=5, fill=tk.X, expand=True)

    # Login button
    login_button = CTkButton(group3, text="Login", command=getvals_login)
