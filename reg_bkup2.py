...
def create_homepage_window(login_window):
    ...
    go_back_button = CTkButton(button_frame, text="Go Back to Login", command=lambda: go_back_to_login(homepage, login_window))
...
# def go_back_to_login(current_window, login_window):
#     current_window.withdraw()  # Hide the current window
#     login_window.deiconify()  # Make the login window visible again

def go_back_to_login(current_window, login_window):
    current_window.withdraw()  # Hide the current window
    
    login_window = CTk()  # Recreate the login window if it has been destroyed
